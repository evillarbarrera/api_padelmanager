// Helper to send notifications for a specific reservation
function sendReservationNotifications($conn, $jugador_id, $reserva_id) {
...
function fulfillPayment($conn, $data) {
    ...
    $tipo = $resPack['tipo'];
    $isGroup = ($tipo === 'grupal' || $tipo === 'pack_grupal' || $tipo === 'clase grupal');
    ...
    if ($isGroup) {
        ...
        try {
            ...
            $sqlBuy = "INSERT INTO pack_jugadores (pack_id, jugador_id, sesiones_usadas, fecha_inicio, fecha_fin, reserva_id, cupon_id, precio_pagado, moneda, metodo_pago, paypal_order_id, comision_plataforma, created_at) VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $stmtBuy = $conn->prepare($sqlBuy);
            $stmtBuy->bind_param("iissiiidssd", $pack_id, $jugador_id, $fecha_inicio, $fecha_fin, $reserva_id, $cupon_id, $amount, $moneda, $metodo, $order_id, $comision);
            $stmtBuy->execute();
            ...
            // NEW: Handle specific reservation activation for group classes
            if ($reserva_id) {
                // Update reservation status, pack link and TYPE
                $stmtRes = $conn->prepare("UPDATE reservas SET estado = 'reservado', pack_id = ?, pack_jugador_id = ?, tipo = 'grupal' WHERE id = ?");
                $stmtRes->bind_param("iii", $pack_id, $pack_jugador_id, $reserva_id);
                $stmtRes->execute();

                // Fulfill player in reserva_jugadores if not exists
                $stmtCheckRJ = $conn->prepare("SELECT id FROM reserva_jugadores WHERE reserva_id = ? AND jugador_id = ?");
                $stmtCheckRJ->bind_param("ii", $reserva_id, $jugador_id);
                $stmtCheckRJ->execute();
                if ($stmtCheckRJ->get_result()->num_rows == 0) {
                    $stmtInsRJ = $conn->prepare("INSERT INTO reserva_jugadores (reserva_id, jugador_id) VALUES (?, ?)");
                    $stmtInsRJ->bind_param("ii", $reserva_id, $jugador_id);
                    $stmtInsRJ->execute();
                }

                // Call the helper for notifications
                sendReservationNotifications($conn, $jugador_id, $reserva_id);
            }
            ...
            // NEW: Handle specific reservation notifications if present
            if ($reserva_id) {
                sendReservationNotifications($conn, $jugador_id, $reserva_id);
            }
        ...
        // Types: i (pack), i (jugador), s (inicio), s (fin), i (reserva), i (cupon), d (precio), s (moneda), s (metodo), s (paypal), d (comision)
        $stmt->bind_param("iissiiidssd", $pack_id, $jugador_id, $fecha_inicio, $fecha_fin, $reserva_id, $cupon_id, $amount, $moneda, $metodo, $order_id, $comision);
        ...
        if ($stmt->execute()) {
            ...
            if ($reserva_id) {
                // Determine session type (individual or multi) - we use 'individual' as standard for these
                $stmtRes = $conn->prepare("UPDATE reservas SET estado = 'reservado', pack_id = ?, pack_jugador_id = ?, tipo = 'individual' WHERE id = ?");
                $stmtRes->bind_param("iii", $pack_id, $pack_jugador_id, $reserva_id);
                $stmtRes->execute();
